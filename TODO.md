...
1. [ ] Make aborts and subprocess failures preserve structured details.
...
2. [ ] Extract a deep execution orchestration module.
...
3. [ ] Clarify and test workspace/CWD semantics.
...
4. [ ] Deepen project-agent trust diagnostics.
...
5. [ ] Introduce a result-summary policy seam.
...
6. [ ] Explore optional fanout-then-reduce orchestration.
...
7. [ ] Add optional per-task runtime controls.
...
8. [ ] Improve README with advanced workflow recipes.
...
9. [ ] Consider moving task prompts out of process argv.
...
10. [ ] Add local install smoke before release.
   - Run `pi install .`, restart or `/reload`, then confirm the `subagent` tool appears and can run a trivial user agent.

11. [ ] Choose the next release version and move `CHANGELOG.md` `Unreleased` entries into that release section before publishing/tagging.
